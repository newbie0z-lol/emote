...
local function fetchAllEmotes()
    ...
    getgenv().Notify({
        Title = 'Rez Hub | Emote',
        Content = " Load Thành Công! Total Emotes: " .. totalEmotesLoaded,
        Duration = 5
    })
...
local function searchEmotes(searchTerm)
    if isLoading then
        getgenv().Notify({
            Title = 'Rez Hub | Emote',
            Content = 'Đang load , vui lòng chờ',
            Duration = 5
        })
        return
    end
...
local function onCharacterAdded(character)
 ...
 if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
    ...
    getgenv().Notify({
        Title = 'Rez Hub | Auto Reload Animation',
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
    ...
    getgenv().Notify({
        Title = 'Rez Hub | Emote Mobile',
        Content = 'mở khi cần sử dụng',
        Duration = 5
    })
end
...
if UserInputService.KeyboardEnabled then
    getgenv().Notify({
        Title = 'Rez Hub | Emote PC',
        Content = ' Mở menu thì nhấn nút "."',
        Duration = 5
    })
end
